# CFnew - 终端 v2.9.6（macOS 风重制版）

> **基于 [byJoey/cfnew](https://github.com/byJoey/cfnew) v2.9.6 修改**
>
> 本项目仅重构 UI 视觉层，业务逻辑（WebSocket 代理、订阅生成、KV 配置、API 管理）与原版完全一致。
>
> - **原项目**：https://github.com/byJoey/cfnew
> - **本项目**：https://github.com/RemotePinee/cfnew

## UI 重构说明

将原版的"绿色霓虹终端"换成 **macOS System Utility 风**，更适合长时间使用：

- 配色：深灰 `#1d1d1f` 底，灰阶分级，蓝色 `#0a84ff` 强调（macOS 系统蓝），仅在状态指示用绿色
- 字体：Inter 正文 + JetBrains Mono 数据/代码（原 Courier New + 绿色霓虹替换）
- 布局：240px 侧栏（10 个客户端竖排）+ 主内容区，订阅链接、系统状态、配置、延迟测试分区清晰
- 删除：matrix 雨、毛玻璃、发光、脉冲动画、扫描线 —— 安静专业，无装饰干扰
- 保留：完整中文 / 波斯语双语 + RTL、所有原有功能（不动一行业务代码）
...
不部署到 Cloudflare 也能在本地浏览器看 UI 效果（仅渲染 HTML/CSS/JS，不跑 worker 业务）：

```bash
node preview.mjs
```

打开 http://localhost:8787/ — 4 个路由：登录页 / 主面板，中文 / 波斯语各一份。直接编辑 `明文源吗`，刷新浏览器即可看到效果，无需重启。
